...
export default function RegistrationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(errorObj.response?.data?.message ?? "Terjadi kesalahan. Silakan coba lagi.");
    } finally {
      setIsLoading(false);
    }
  };
            ...
            <div className="relative flex items-center gap-3">
              <div className="w-11 h-11 rounded-xl bg-white/15 backdrop-blur-sm flex items-center justify-center">
                <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6m2 0v-6a2 2 0 012-2h2a2 2 0 012 2v6m-6 0V9a2 2 0 012-2h2a2 2 0 012 2v6" />
                </svg>
              </div>
              <div>
          ...
          <form className="space-y-4 p-5 sm:p-6" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <input
                ...
                className={fieldClasses}
              />
              <p className="mt-1.5 text-xs text-slate-400">QR code akan dikirim ke email ini</p>
            </div>
            ...
            <div className="flex items-start gap-2.5 rounded-xl bg-blue-50 p-3.5 text-sm text-blue-700 border border-blue-100">
              ...
              <span>Pastikan data Anda benar. QR code akan digunakan untuk absensi.</span>
            </div>
